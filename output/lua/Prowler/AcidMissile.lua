...
AcidMissile.kHitEffect = PrecacheAsset("cinematics/alien/structures/death_small.cinematic")
AcidMissile.kHitSoundEffect = PrecacheAsset("sound/NS2.fev/alien/structures/death_small")

AcidMissile.kRadius             = 0.3
AcidMissile.kClearOnImpact      = true
AcidMissile.kClearOnEnemyImpact = true
AcidMissile.kSplashRadius       = 1.0 --2.0
AcidMissile.kDamage             = kAcidSprayDamage

-- The max amount of time a AcidMissile can last for
AcidMissile.kLifetime = 0.40 --5
...
if Server then
    ...
    function AcidMissile:ProcessHit(targetHit, surface, normal)        
        ...
        --self:TriggerEffects("construction_complete", {classname = "Drifter"})
...
function AcidMissile:GetMaxFocusBonusDamage()
    return kAcidSprayFocusDamageBonusAtMax
end

function AcidMissile:GetFocusAttackCooldown()
    return kAcidSprayFocusAttackSlowAtMax
end
...
function AcidMissile:GetVampiricLeechScalar()
    return kAcidSprayVampirismScalar
end
