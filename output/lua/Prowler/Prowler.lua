...
Prowler.kWalkBackwardSpeedScalar = 1.0
Prowler.kHorizontalJumpForce = 2.2 --7.2
Prowler.kVerticalExtraJumpForce = 5 --3.11 --2.92
...
function Prowler:GetMapBlipType()
    return kMinimapBlipType.Drifter --kMinimapBlipType.Prowler --kMinimapBlipType.Skulk
end
...
function Prowler:GetAcceleration()
    return self.rappelling and 6 or 11
end

function Prowler:GetGroundFriction()
    return self.rappelling and 3 or 9
end
...
function Prowler:ModifyJump(input, velocity, jumpVelocity)
    ...
    if input.move:GetLength() > 0 then
        ...
        local verticalForce = (self:GetIsOnGround() and jumpVelocity.y or kWallJumpVerticalForce) + Clamp(wishDir.y * 1.2, 0, 1) * (Prowler.kVerticalExtraJumpForce + celerityBonus)
        wishDir.y = 0
        ...
        jumpVelocity.y = verticalForce * verticalModifier
    ...
    elseif self.wallGripping or (Shared.GetTime() - self.timeLastWallWalkCheck < 0.09) then
        ...
        jumpVelocity.y = kWallJumpVerticalForce
